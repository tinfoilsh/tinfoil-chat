...
import type { DocumentProcessingResult, DocumentUploadProps } from './types'
...
export const getFileIconType = (filename: string): string => {
  ...
  if (lowerFilename.endsWith('.xls') || lowerFilename.endsWith('.xlsx')) return 'xlsx';
  if (lowerFilename.endsWith('.csv')) return 'csv';
  
  if (lowerFilename.endsWith('.jpg') || 
      lowerFilename.endsWith('.jpeg') || 
      lowerFilename.endsWith('.png') || 
      lowerFilename.endsWith('.gif')) return 'image';
  
  if (lowerFilename.endsWith('.mp3') || 
      lowerFilename.endsWith('.wav') ||
      lowerFilename.endsWith('.ogg')) return 'audio';
  
  if (lowerFilename.endsWith('.mp4') || 
      lowerFilename.endsWith('.mov') ||
      lowerFilename.endsWith('.avi')) return 'video';
  
  if (lowerFilename.endsWith('.zip') || 
      lowerFilename.endsWith('.rar') ||
      lowerFilename.endsWith('.tar')) return 'zip';
  ...
  const handleDocumentUpload = async (
    ...
    try {
      ...
      if (file.size > CONSTANTS.MAX_DOCUMENT_SIZE_BYTES) {
        onError(new Error(`File size exceeds the limit of ${CONSTANTS.MAX_DOCUMENT_SIZE_MB}MB.`), documentId);
        return;
      }
      ...
      const response = await fetch('https://doc-upload.model.tinfoil.sh/v1alpha/convert/file', {
        method: 'POST',
        body: formData,
      })
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          throw new Error(`API endpoint not found or not accessible. Please check the API URL.`)
        } else {
          throw new Error(`Document upload failed with status: ${response.status}. ${errorText}`)
        }
      }
      
      // Process the direct response
      const processingResult = await response.json() as DocumentProcessingResult
      ...
      onError(error instanceof Error ? error : new Error('Unknown error during document processing'), documentId)
    } finally {

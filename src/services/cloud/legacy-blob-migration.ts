...
export async function runLegacyBlobMigration(): Promise<MigrationReport> {
  ...
  console.log('[v2-launch] runLegacyBlobMigration: start', {
    ...
    pollIntervalMs: MIGRATION_POLL_INTERVAL_MS,
    pollTimeoutMs: MIGRATION_POLL_TIMEOUT_MS,
  })
  ...
  try {
    ...
    console.log('[v2-launch] runLegacyBlobMigration: kickoff FAILED', {
  ...
  while (lastResp?.partial && (lastResp.status ?? 'running') === 'running') {
    if (Date.now() - startedAt > MIGRATION_POLL_TIMEOUT_MS) {
      console.log('[v2-launch] runLegacyBlobMigration: poll timeout', {
        elapsedMs: Date.now() - startedAt,
    ...
    try {
      ...
      console.log('[v2-launch] runLegacyBlobMigration: poll FAILED', {
        elapsedMs: Date.now() - startedAt,
        error: err instanceof Error ? err.message : String(err),
      })
    ...
    console.log('[v2-launch] runLegacyBlobMigration: poll tick', {
      elapsedMs: Date.now() - startedAt,
      status: lastResp.status,
      partial: lastResp.partial,
      scopes: lastResp.scopes.map((s) => ({
        scope: s.scope,
        migrated: s.migrated,
        remaining: s.retryable_remaining,
        blockedUnmigrated: s.blocked_unmigrated,

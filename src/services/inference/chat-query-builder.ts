...
const GENUI_TOOL_HINT = `You have render_* tools for rich visual components: charts (bar, line, pie, area), tables (data, comparison), source cards and link previews for web results, timelines, stat cards, progress bars, callouts (info/warning/tip/success/error), key-value lists, info cards, steps/checklists, image grids, and render_image for a single image/diagram (accepts hosted URLs, inline SVG, base64 bytes, or Mermaid diagram source). When content benefits from visual structure — comparisons, trends, proportions, chronologies, curated sources, highlighted takeaways, structured facts, diagrams, or illustrations — always call the appropriate render tool instead of markdown, mermaid, or code blocks. Use render_image for generated SVG/Mermaid diagrams and hero images; use render_image_grid for galleries. After using web search, prefer render_source_cards or render_link_preview to surface the best sources. You may call multiple render tools in one response.`
